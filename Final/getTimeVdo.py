...
def hough_transform(frame, src):
    ...
    # Loads an image
    # img = cv.imread(cv.samples.findFile(frame), cv.IMREAD_COLOR)
...
def line_transform(crop_img):
    ...
    # canny_img = cv.dilate(canny_img, None, iterations=1)
    ...
    minute_hand = []
    hour_hand = []
    if linesP_img is not None and len(linesP_img) > 0:
        max_length = 0
...
def initializeTrackbars(intialTracbarVals=0):
    ...
    cv.createTrackbar("Minrad", "Trackbars", 200, 300, nothing)
    cv.createTrackbar("Maxrad", "Trackbars", 275, 300, nothing)




def valTrackbars():
    Minrad = cv.getTrackbarPos("Minrad", "Trackbars")
    Maxrad = cv.getTrackbarPos("Maxrad", "Trackbars")
    src = Minrad, Maxrad
    return src
...
while True:
    if __name__ == "__main__":
        ...
        if frame is not None:
            crop_img, check = hough_transform(frame, src)
            if check:
                angles = line_transform(crop_img)
                if len(angles) > 0:
                    h, m = getTime(angles)
                    text = 'Time : ' + h + ':' + m
                    cv.putText(frame, text, (10, 50), cv.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv.LINE_AA)
